# C3-14-Multi-AZ、Multi-Region-与灾备
...
| 层/服务 | 解决的问题 | 为什么放在这里 |
|---|---|---|
| Multi-AZ | Region 内故障隔离 | 首先处理单 AZ/组件故障 |
| Multi-Region | Region 级故障与全球需求 | 更大故障域但复杂度/成本更高 |
| RTO | 最多允许停多久 | 决定备用环境恢复速度 |
| RPO | 最多允许丢多少数据 | 决定复制/备份频率 |
| Route 53 | DNS Failover/路由 | 把用户切换到健康 Region |
| Backup/Replication | 恢复数据 | 不是只有 Compute 需要 DR |
...
Multi-AZ 典型解决高可用；Region 级灾备需要额外 Multi-Region 设计。
...
Replication 复制最新状态也可能复制错误；Backup 提供历史恢复点。
