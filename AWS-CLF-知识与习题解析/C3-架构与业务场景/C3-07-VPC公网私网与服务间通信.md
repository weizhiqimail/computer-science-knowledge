...
从“谁能访问谁”重新理解 VPC，而不是只背 Public/Private Subnet 名称。
...
Private App Subnet A/B
  │  EC2/ECS
  ├──→ VPC Endpoint → S3/DynamoDB/Private services
  ▼
Private DB Subnet A/B
     RDS/Aurora

Security Group：资源级有状态
NACL：Subnet 级无状态
Route Table：决定下一跳
```
...
| 层/服务 | 解决的问题 | 为什么放在这里 |
|---|---|---|
| VPC/Subnet | 网络隔离和地址分段 | 先定义逻辑网络边界 |
| Route Table | 决定流量下一跳 | Public/Private 本质来自路由和地址条件 |
| Internet Gateway | VPC 公网连接点 | Public workload 对公网通信 |
| NAT Gateway | Private IPv4 出网 | 允许主动访问外网但不直接接受公网入站 |
| Security Group | 资源级有状态防火墙 | 精确控制应用到数据库等访问 |
| NACL | Subnet 级无状态过滤 | 第二层粗粒度网络控制 |
| VPC Endpoint/PrivateLink | 私网访问 AWS/私有服务 | 减少公共 Internet 路径 |
...
### 5.1-Public-Subnet-是名字决定的吗？

不是，关键看路由和资源公网地址/入口条件。

### 5.2-数据库为什么放-Private-Subnet？

数据库不需要直接面对公网，只允许应用层访问更符合最小暴露原则。

### 5.3-SG-与-NACL-为什么都有？

它们作用层级和状态模型不同，可以形成纵深防御。

### 5.4-NAT-Gateway-与-Internet-Gateway-怎么区分？

IGW 是 VPC 公网连接；NAT 常为 Private Subnet 提供出站地址转换。
...
- Route Table 错误：服务可能完全不可达，即使 SG 已允许。
- SG 过宽：暴露面扩大。
- NACL 拒绝返回流量：无状态规则需要同时考虑双向。
- Private 资源依赖公网 AWS API：可考虑 VPC Endpoint 减少公网路径。
...
- C2-06：VPC/Subnet/Route/IGW/NAT/SG/NACL/Endpoint
- C2-08：IAM 是权限层，不替代网络层
- C2-09：Defense in Depth

## 9-与后续-C4-题库解析的关系

C3 不以背题库答案为目标。进入 C4 后，同一个场景会被拆成很多单点选择题。例如架构图里只有一条“用户访问链路”，考试可能分别问 DNS、CDN、WAF、Load Balancer、Auto Scaling、数据库、Cache、监控。
