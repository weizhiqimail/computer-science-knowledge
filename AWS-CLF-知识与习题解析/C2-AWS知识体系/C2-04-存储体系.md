...
```text
数据的生命周期
不应该仅仅绑定在某个 Application Process
或者某一次临时运行上。
```
...
```text
globalshop-product-images
globalshop-order-exports
globalshop-access-logs
```
...
product-12345.jpg
manual.pdf
invoice-20260905.pdf
access-log-2026-09-05.gz
backup-file.tar
```
...
```text
Object 在 Bucket 中的唯一名称 / 标识
```

例如：

```text
products/12345/main.jpg
```
...
```text
多个 Linux Server 共享 NFS File System
```
...
AWS 对多个 S3 Storage Class 设计的对象耐久性目标是：
...
S3 Intelligent-Tiering 的核心问题是：

```text
我不知道数据以后到底访问得频繁还是不频繁。
```
...
```text
automatically move old objects
...
```text
给 EC2 提供可配置、持久化的 Block Storage Volume。
```
...
但用户不是去某个普通 S3 Bucket 里直接操作 Snapshot 文件。
...
CLF-C02 不要求像 Solutions Architect 那样深入记：
...
```text
发明一种新的 Block / File / Object Storage
```
...
持续连接
Hybrid Storage
本地应用长期通过 Gateway 使用 AWS Storage
```
...
S3 也可以保存文件内容
FSx 也是文件存储
Storage Gateway File Gateway 也处理文件协议
```
...
[AWS 官方文档：What is Amazon S3?](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html)
...
[AWS 官方文档：Amazon EBS volumes](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-volumes.html)

EBS Snapshots：

[AWS 官方文档：Amazon EBS snapshots](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-snapshots.html)
...
[AWS 官方文档：What is Amazon Elastic File System?](https://docs.aws.amazon.com/efs/latest/ug/whatisefs.html)

Amazon FSx for Windows File Server：

[AWS 官方文档：What is FSx for Windows File Server?](https://docs.aws.amazon.com/fsx/latest/WindowsGuide/what-is.html)
