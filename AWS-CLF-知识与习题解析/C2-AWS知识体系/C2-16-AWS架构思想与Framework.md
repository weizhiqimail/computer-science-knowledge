...
前面 C2-01～C2-15 学了大量具体服务。这最后一章不再增加一堆产品，而是回答：
...
Reliability 不等于“绝不发生故障”。成熟架构默认组件会失败，并设计检测、隔离、恢复和冗余机制。
...
Cost Optimization ≠ “最低成本”。一个省钱但无法满足 SLA 的方案不是好架构。
...
AWS 管理更多底层计算与 OS；客户聚焦函数代码、依赖、IAM 权限、数据和业务安全。
